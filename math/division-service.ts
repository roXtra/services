...
export async function serviceLogic(environment: IServiceTaskEnvironment): Promise<void> {
  ...
  if (((environment.instanceDetails.extras.fieldContents[numberField2] as IFieldValue).value as number) === 0) {
    return;
  }
...
export async function division(environment: IServiceTaskEnvironment): Promise<boolean> {
  try {
