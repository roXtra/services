...
export async function serviceLogic(environment: PH.ServiceTask.ServiceTaskEnvironment): Promise<void> {
  ...
  const taskObject: Bpmn.Task = processObject.getExistingTask(processObject.processId(), environment.bpmnTaskId);
  const extensionValues: PH.Process.TaskExtensions = PH.Process.BpmnProcess.getExtensionValues(taskObject);
