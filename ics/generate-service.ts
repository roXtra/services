...
export async function generate(environment: IServiceTaskEnvironment): Promise<boolean> {
  ...
  const url: string = await environment.instances.uploadAttachment(instance.processId, instance.instanceId, "ics_import_file.ics", Buffer.from(icsString).toString("base64"));
