...
import { getFieldKey, getLaneKey } from "./utils/field-resolver.js";
...
export async function serviceLogic(environment: IServiceTaskEnvironment): Promise<void> {
  ...
  const options: IGenerateXLSXOptions = {
    language,
    module: {
      name: processDetails.urlName === "actionmanagement-basic" ? "action" : "action_basic",
      urlPrefix: processDetails.urlName === "actionmanagement-basic" ? "m" : "mp",
      title: processDetails.urlName === "actionmanagement-basic" ? "Maßnahmen" : "Maßnahmen Plus",
    },
  ...
  environment.logger.debug(`Instance23 keys: ${viewColumns.map((col) => col.field).join(", ")}`);
  filteredInstances.forEach((instance) => {
    environment.logger.debug(`Instance23 values (direct): ${instance.instanceId} - ${JSON.stringify(instance)}`);
  });
  filteredInstances.forEach((instance) => {
    environment.logger.debug(`Instance23 values (fieldContents): ${instance.instanceId} - ${JSON.stringify(instance.extras?.fieldContents)}`);
  });
...
export async function measures(environment: IServiceTaskEnvironment): Promise<boolean> {
